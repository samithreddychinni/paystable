...
func TestIndependentReportUsesUnseenImplementations(t *testing.T) {
	...
	if len(first.Cases) != 16 {
		t.Fatalf("independent report has %d cases, want 16", len(first.Cases))
	}
	...
	for _, estimate := range first.Confidence {
		if estimate.Method == BaselineRandom && (estimate.Trials != 160 || estimate.FalseFindingCount != 0) {
